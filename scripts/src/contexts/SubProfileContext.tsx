...
export function SubProfileProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!parentKeypair) {
      // No parent keypair means this is a post-subprofile account — nothing to heal.
      localStorage.setItem(healFlag, "1");
      return;
    }

    // Migrate the parent keypair and ratchet states to ALL sub-profiles that
    // are missing their identity keypair (not just subProfiles[0]).  Previously
    // this only copied to the first sub-profile; users with more than one
    // sub-profile created before the sub-profile system was fully adopted would
    // have their later sub-profiles without any keypair after restore.
    let didAnything = false;
    const ratchetPrefix = `ratchet_v3_${parentHash}_`;

    for (const sub of subProfiles) {
      const newHash = sub.identityHash.toLowerCase();
      ...
      // Copy ratchet state from parent → sub-profile namespace if not already there.
      const keysToCopy: Array<[string, string]> = [];
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (!k || !k.startsWith(ratchetPrefix)) continue;
        const peerHash = k.slice(ratchetPrefix.length);
