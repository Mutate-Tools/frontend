...
const InboxRefer = () => {
  ...
  const displayedApprovedMute =
    (referrals?.claimableMuteBalance ?? 0) + (referrals?.pendingJoinMuteBalance ?? 0);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="bg-[#FFFFFF0D] border border-white/10 rounded-2xl p-4 flex flex-col gap-2">
            ...
            <p className="text-white text-[28px] font-sora font-semibold">
              {loading ? <SkeletonCardValue /> : displayedApprovedMute.toLocaleString()}
            </p>
            <button
              onClick={handleClaim}
